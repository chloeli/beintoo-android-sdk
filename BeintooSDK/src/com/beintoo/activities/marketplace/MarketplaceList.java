...
public class MarketplaceList extends BeintooListViewDialogES implements Button.OnClickListener{
	...
	public MarketplaceList(Context context) {
		super(context);
		...
		loadMarketplace(MarketplaceList.MarketplaceKind_NATIONAL_TOPSOLD, null, null);		
	}
	...
	private void setupBalanceStatus(){
		...
		if(Beintoo.virtualCurrencyData != null){ // if the app is with virtual currency add the player virtual currency balance
			MarketplaceList.userBedollars = Double.parseDouble(Beintoo.virtualCurrencyData.get("currencyBalance"));
			...
			MarketplaceList.userBedollars = player.getUser().getBedollars();
			...
			MarketplaceList.userBedollars = -0.1;
	...
	public void onClick(View v) {		
		...
		if(v.getId() == R.id.marketplace_featured_button){
			...
			hideSubcategory();
			hidePriceFilter();
			loadMarketplace(MarketplaceList.MarketplaceKind_FEATURED, MarketplaceList.MarketplaceSort_PRICE_ASC, null);
			v.requestFocus();
			...
			showSubcategory();
			hidePriceFilter();
			loadMarketplace(MarketplaceList.MarketplaceKind_NATIONAL_TOPSOLD, MarketplaceList.MarketplaceSort_PRICE_ASC, null);
		}else if(v.getId() == R.id.marketplace_aroundme_button){
			mCurrentSection = R.id.marketplace_aroundme_button;
			hideSubcategory();
			hidePriceFilter();
			loadMarketplace(MarketplaceList.MarketplaceKind_AROUNDME, MarketplaceList.MarketplaceSort_DISTANCE,null);
			...
			if(mCurrentSection == R.id.marketplace_top_button){
				...
				loadMarketplace(MarketplaceList.MarketplaceKind_NATIONAL, MarketplaceList.MarketplaceSort_PRICE_ASC, mCurrentCategory);
			}
		}else if(v.getId() == R.id.marketplace_order_desc){
			if(mCurrentSection == R.id.marketplace_top_button){
				loadMarketplace(MarketplaceList.MarketplaceKind_NATIONAL_TOPSOLD, MarketplaceList.MarketplaceSort_PRICE, null);
			}else if(mCurrentSection == R.id.marketplace_featured_button){
				loadMarketplace(MarketplaceList.MarketplaceKind_FEATURED, MarketplaceList.MarketplaceSort_PRICE, null);
			}else if(mCurrentSection == R.id.marketplace_category_button){
				loadMarketplace(MarketplaceList.MarketplaceKind_NATIONAL, MarketplaceList.MarketplaceSort_PRICE, mCurrentCategory);
			}
		}
	}
	...
	private void updateVirtualCurrencyView(){
		...
		((TextView) findViewById(R.id.marketplace_balancecredit)).setText(MarketplaceList.userBedollars+" "+currencyName);
		if(objectList.size() > 0){
	...
	public static void updateVirtualCurrency(Double creditsSpent){
		MarketplaceList.userBedollars = MarketplaceList.userBedollars - creditsSpent;
		Beintoo.updateVirtualCurrencyBalance(MarketplaceList.userBedollars);
	}
